...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ] ;then
        echo "hyprshade_filter=\"$choice\"" > ~/.config/HyprSetup/settings/hyprshade.sh
        if [ "$choice" == "off" ] ;then
            hyprshade off
    ...
    if [ -f ~/.config/HyprSetup/settings/hyprshade.sh ] ;then
        source ~/.config/HyprSetup/settings/hyprshade.sh
    fi
